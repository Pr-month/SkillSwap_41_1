...
## Фича: обновление пароля пользователя

### Эндпоинт
- `PATCH /users/me/password`

### Назначение
Позволяет авторизованному пользователю обновить свой пароль, не затрагивая профильные данные.
...
### Правила обработки
- запрос доступен только под `AccessTokenGuard`
- текущий пользователь берётся из JWT payload (`sub`)
- старый пароль проверяется через `bcrypt.compare`
- новый пароль сохраняется только в виде хеша
- в ответ возвращается безопасное сообщение об успехе без пароля и токенов

### Валидация
- новый пароль должен быть строкой
- новый пароль должен быть не пустым
- минимальная длина нового пароля — 8 символов
- максимальная длина нового пароля — 255 символов
