...
## Фича: создание заявки

### Эндпоинт
- `POST /requests`

### Назначение
Создаёт новую заявку на обмен навыками от текущего авторизованного пользователя к выбранному получателю.
...
### Тело запроса
- `receiverId` — UUID пользователя-получателя
- `offeredSkillId` — UUID навыка, который предлагает отправитель
- `requestedSkillId` — UUID навыка, который он хочет получить

### Поведение
- создаётся заявка со статусом `pending`
- `isRead` устанавливается в `false`
- запрос проходит проверки на существование пользователей и навыков
- навык должен принадлежать нужному пользователю
- нельзя отправить заявку самому себе

### Ответ
Возвращается созданная заявка с безопасно выбранными полями:
- отправитель
- получатель
- предлагаемый навык
- запрашиваемый навык
...
- e2e-тест маршрута `POST /requests`
