...
## Фича: получение входящих заявок

### Эндпоинт
- `GET /requests/incoming`

### Назначение
Возвращает список заявок, где текущий авторизованный пользователь является получателем.

### Доступ
- только для авторизованного пользователя
- запрос защищён `AccessTokenGuard`
- идентификатор пользователя берётся из JWT payload (`sub`)

### Что возвращает
- список входящих заявок текущего пользователя
- заявки отсортированы по дате создания, новые — первыми
- в ответ включаются связанные данные по отправителю, получателю и навыкам

### Безопасность
- сервис отбирает только заявки текущего получателя
- в ответ не должны попадать лишние приватные данные

### Проверка
- unit-тесты для сервиса и контроллера
- e2e-тест маршрута `GET /requests/incoming`
