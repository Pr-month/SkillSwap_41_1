...
## Фича: удаление навыка из избранного

### Эндпоинт
- `DELETE /skills/:id/favorite`

### Назначение
Удаляет выбранный навык из списка избранных текущего авторизованного пользователя.
...
### Поведение
- навык удаляется только из `favoriteSkills` текущего пользователя
- если пользователь или навык не найдены, возвращается ошибка
- если навык не находится в избранном, возвращается ошибка
- в ответе возвращается безопасное сообщение об успехе

### Проверка
- unit-тесты для контроллера и сервиса
- e2e-тест маршрута `DELETE /skills/:id/favorite`
